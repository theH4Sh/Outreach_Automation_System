          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div className="max-w-2xl">
                <h3 className="text-2xl font-bold text-slate-900">{displayedCampaign.name}</h3>
                <p className="mt-2 text-slate-600">{displayedCampaign.description || 'No campaign description available.'}</p>
              </div>

              <div className="flex flex-col gap-3 sm:items-end">
                <span className={`inline-flex items-center rounded-full px-4 py-1.5 text-sm font-semibold ${statusBadge(displayedCampaign.status)}`}>
                  {displayedCampaign.status}
                </span>
                {displayedCampaign.browserProfile && (
                  <span className="inline-flex items-center rounded-full bg-slate-100 px-4 py-1.5 text-sm font-medium text-slate-700">
                    {displayedCampaign.browserProfile.profileName || displayedCampaign.browserProfile}
                  </span>
                )}
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-slate-600">Campaign progress</p>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl bg-indigo-50 p-4 text-center">
                <p className="text-xs uppercase tracking-wider text-indigo-400 font-medium">Progress</p>
                <p className="mt-1 text-2xl font-bold text-indigo-700">{progressPercentage}%</p>
              </div>
              <div className="rounded-2xl bg-violet-50 p-4 text-center">
                <p className="text-xs uppercase tracking-wider text-violet-400 font-medium">Leads</p>
                <p className="mt-1 text-2xl font-bold text-violet-700">{displayedCampaign.leads?.length ?? 0}</p>
              </div>
              <div className="rounded-2xl bg-emerald-50 p-4 text-center">
                <p className="text-xs uppercase tracking-wider text-emerald-400 font-medium">Created</p>
                <p className="mt-1 text-2xl font-bold text-emerald-700">{displayedCampaign.createdAt ? new Date(displayedCampaign.createdAt).toLocaleDateString() : 'N/A'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-lg font-bold text-slate-900">Live campaign logs</h4>
              </div>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{logs.length} events</span>
            </div>
            {logs.length === 0 ? (
